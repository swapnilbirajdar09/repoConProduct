...
function openForm(modal_id,activity_id) {
    var modal = $('#' + modal_id);
    modal.addClass('in');
    getAllQueries(activity_id);
}
//----------fun for add comment
...
function getAllQueries(activity_id) {
    $('#query_list_' + activity_id).html("<span class='w3-text-grey'><i class='fa fa-circle-o-notch fa-spin'></i> Loading Queries. Please wait... </span>");
    $.ajax({
        type: "GET",
        url: BASE_URL + "modules/site_inspection/getAllQueries",
        data: {
            activity_id: activity_id
        },
        cache: false,
        success: function (response) {
            //alert(response);
            // console.log(response);
            $('#query_list_' + activity_id).html(response);
            // var data = JSON.parse(response);
        }
    });
}
